...
// DC     接PB4
// CS     如有则默认直接接地
#define OLED_DC_CLS() PBout(4)=0 //DC
#define OLED_DC_SET() PBout(4)=1 //DC

#define OLED_RST_CLS() PCout(13)=0 //RST
#define OLED_RST_SET() PCout(13)=1 //RST

#define OLED_SCLK_CLS() PCout(15)=0 //SCL
#define OLED_SCLK_SET() PCout(15)=1 //SCL

#define OLED_SDIN_CLS() PCout(14)=0 //SDA
#define OLED_SDIN_SET() PCout(14)=1 //SDA

#define OLED_CS_CLS()// CS
#define OLED_CS_SET()
...
// 8*16点阵ASCII
#define F8X16_WIDTH 8
#define F8X16_HEIGHT 16
const u8 F8X16[] = {
...
// 设置显示位置
void oled_set_pos(u8 pos_x, u8 pos_y) {
    oled_write_byte(0xb0 + pos_y, OLED_CMD);
    oled_write_byte(((pos_x & 0xf0) >> 4) | 0x10, OLED_CMD);
    oled_write_byte((pos_x & 0x0f) | 0x01, OLED_CMD);
}
...
// mode: 0,反白显示;1,正常显示
// size: 选择字体 16/12
void oled_show_char(u8* pos_x, u8 *pos_y, u8 ch) {
    unsigned char c = 0, i = 0;
    c = ch - ' '; // 得到ASCII偏移后的值
    if (FONT_SIZE == 0) {
        if (*pos_x + F6X8_WIDTH > SCREEN_WIDTH || ch == '\n') {
            *pos_x = 0;
            *pos_y = *pos_y + 1;
            if (ch == '\n') {
                return;
        ...
        if (*pos_y * 8 > SCREEN_HEIGHT - F6X8_HEIGHT) {
            return;
        }
        oled_set_pos(*pos_x, *pos_y + 1);
        for (i = 0; i < 6; i++) {
            oled_write_byte(F6X8[c][i], OLED_DATA);
        }
        *pos_x = *pos_x + F6X8_WIDTH;
    } else {
        if (*pos_x + F8X16_WIDTH > SCREEN_WIDTH || ch == '\n') {
            *pos_x = 0;
            *pos_y = *pos_y + 2;
            if (ch == '\n') {
                return;
        ...
        if (*pos_y * 8 > SCREEN_HEIGHT - F8X16_HEIGHT) {
            return;
        }
...
// 显示字符串
void oled_show_string(u8 pos_x, u8 pos_y, u8 *show_string) {
    unsigned char i = 0;
    while (show_string[i] != '\0') {
        oled_show_char(&pos_x, &pos_y, show_string[i]);
        i++;
    }
}
...
void oled_init(void) {
    ...
    RCC_LSEConfig(RCC_LSE_OFF); // 外部低俗振荡器关闭 PC14，PC15成为普通IO，侵入检测TAMPER引脚作为通用IO口使用
    PWR_BackupAccessCmd(DISABLE); //备份区写保护

    GPIO_PinRemapConfig(GPIO_Remap_SWJ_JTAGDisable, ENABLE); // PB4成为普通IO
