...
uint8_t icm20602_get_acc_gyro_with_calib(float *ax, float *ay, float *az, float *gx, float *gy, float *gz, int8_t *is_dynamic, int8_t *is_calibrated) {
    // acc
    const int16_t ACC_FIFO_SIZE = 5, /*ACC_DYN_DET_TH = 25*/ACC_DYN_DET_TH = 18, ACC_DYN_BWIDTH = 4, ACC_NO_DYN_DET_TIME = 3;
    ...
    int8_t is_acc_dyn;
    // gyro
    const int16_t GYRO_FIFO_SIZE = 25, /*GYRO_DYN_DET_TH = 9*/GYRO_DYN_DET_TH = 18, GYRO_OFFSET_TH_MAX = 163, GYRO_OFFSET_TH_STEP = 4;
    int16_t GYRO_NO_DYN_DET_TIME = 25;
    ...
    if (!is_acc_dyn && !is_gyro_dyn) {
        ...
        gyro_dyn_det_count = 0;
        if (GYRO_NO_DYN_DET_TIME > GYRO_FIFO_SIZE) {
            GYRO_NO_DYN_DET_TIME = GYRO_FIFO_SIZE;
        }
        gx_fifo_mean = MATH_Mean(gx_fifo, 0, GYRO_NO_DYN_DET_TIME);
        gy_fifo_mean = MATH_Mean(gy_fifo, 0, GYRO_NO_DYN_DET_TIME);
        gz_fifo_mean = MATH_Mean(gz_fifo, 0, GYRO_NO_DYN_DET_TIME);
    ...
    *is_calibrated = is_gyro_calibrated;
    //printf("acc_dyn:%d gyro_dyn:%d max x %d %d y %d %d z %d %d gyro %f %f %f\n", is_acc_dyn, is_gyro_dyn, gx_max, gx_min, gy_max, gy_min, gz_max, gz_min, *gx, *gy, *gz);
    return 0;
}
