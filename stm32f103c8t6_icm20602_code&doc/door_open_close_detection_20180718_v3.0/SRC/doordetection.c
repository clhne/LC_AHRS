...
int Door_Detection(short ax_adc, short ay_adc, short az_adc, short gx_adc, short gy_adc, short gz_adc, float *pitch, float *roll, float *cor_gx, float *cor_gy, float *cor_gz,
	  float *cor_ax, float *cor_ay, int *peak_trough_index, int *peak_trough_count, long long ts)
{
	  int door_status;
    ...
    if (NDOF_DoStep(ax_adc, ay_adc, az_adc, gx_adc, gy_adc, gz_adc, ts)) {
        ...
        //seq/array monotonicity detection
			if(fabs(*cor_ax) > Model_X_ax){
				Door_angle = fabs(*pitch);
			}
			if(fabs(*cor_ay) > Model_Y_ay){
