        ...
        if (icm20602_get_acc_gyro_adc(&ax_adc, &ay_adc, &az_adc, &gx_adc, &gy_adc, &gz_adc)) {
            ...
            prev_ts = millis();
					  NDOF_GetCorAccData(&cor_ax, &cor_ay,&cor_az);
						...
						door_status = Door_Detection(ax_adc, ay_adc, az_adc, gx_adc, gy_adc, gz_adc, &pitch, &roll, &yaw, &cor_gx, &cor_gy, &cor_gz, &cor_ax,&cor_ay,
					                 &peak_trough_index, &peak_trough_count,prev_ts,&Door_angle);		
					  ...
					  printf("door_angle %f,%f,%f,%f,pitch %f,roll %f,yaw %f\n",Door_angle,cor_ax,cor_ay,cor_az,pitch,roll,yaw);
            //door_status = Door_Detection(ax_adc, ay_adc, az_adc, gx_adc, gy_adc, gz_adc, &pitch, &cor_gx, &cor_gy, &cor_gz, &peak_trough_index, &peak_trough_count,prev_ts);
					  cur_ts = millis();
