    ...
    while(1) {
        ...
        if (icm20602_get_accel(cur_acc) == 0 && icm20602_get_gyro(cur_gyro) == 0 && ak8975_get_mag(cur_mag) == 0) {
            ...
            if(is_first_time) {
                ...
                //MadgwickAHRSupdateIMU(cur_gyro[1], cur_gyro[0], -cur_gyro[2], cur_acc[1], cur_acc[0], -cur_acc[2], dt);
                MadgwickAHRSupdateIMU(cur_gyro[1], cur_gyro[0], -cur_gyro[2], cur_acc[1], cur_acc[0], -cur_acc[2], dt);
                ...
                cur_yaw   = atan2(2.0f * (q1 * q2 + q0 * q3), q0 * q0 + q1 * q1 - q2 * q2 - q3 * q3);
                cur_pitch = -asin(2.0f * (q1 * q3 - q0 * q2));
                cur_roll  = atan2(2.0f * (q0 * q1 + q2 * q3), q0 * q0 - q1 * q1 - q2 * q2 + q3 * q3);
                cur_yaw   *= 180.0f / PI;
                //cur_yaw   -= 13.8f; // Declination at Danville, California is 13 degrees 48 minutes and 47 seconds on 2014-04-04
                cur_pitch *= 180.0f / PI;
                cur_roll  *= 180.0f / PI;
                cost = millis() - cost;
