...
void oled_show_char(u8* pos_x, u8 *pos_y, u8 ch) {
    ...
    if (FONT_SIZE == 0) {
        ...
        if (*pos_x + F6X8_WIDTH > SCREEN_WIDTH || *pos_y * 8 > SCREEN_HEIGHT - F6X8_HEIGHT) {
            return;
        }
        ...
        *pos_x = *pos_x + F6X8_WIDTH;
        ...
        if (*pos_x + F8X16_WIDTH > SCREEN_WIDTH || *pos_y * 8 > SCREEN_HEIGHT - F8X16_HEIGHT) {
            return;
        }
...
// 显示字符串
void oled_show_string(u8 pos_x, u8 pos_y, char *show_string) {
    unsigned char i = 0;
