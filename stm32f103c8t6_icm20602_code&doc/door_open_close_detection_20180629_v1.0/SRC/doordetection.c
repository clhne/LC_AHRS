                ...
                if(door.peak_trough_init == 1) {
                    if(door.cur_monotonicity != door.prev_monotonicity) {
                        if(door.cur_monotonicity == 0 && door.prev_monotonicity == 1 && prev_cor_gx > 0.125) {
                            door.cur_peak_trough_value = prev_cor_gx;
                            if(door.prev_flag_peak_trough == 1) {
                                door.cur_peak_trough_value = MAX(door.prev_peak_trough_value, door.cur_peak_trough_value);
                                if(door.cur_peak_trough_value < 0.25) {
                                    door.prev_dt = millis();
                                }
                            } else {
                                if(fabs(door.cur_peak_trough_value) <= fabs(door.prev_prev_peak_trough_value)) {
                                    door.count_peak_trough++;
                                }
                                door.peak_trough_index ++;
                            }
                            door.cur_flag_peak_trough = 1;
                            door.prev_flag_peak_trough = door.cur_flag_peak_trough;
                            door.prev_peak_trough_value = door.cur_peak_trough_value;
                        } else if(door.cur_monotonicity == 1 && door.prev_monotonicity == 0 && prev_cor_gx <= -0.125) {
                            door.cur_peak_trough_value = fabs(prev_cor_gx);
                            if(door.prev_flag_peak_trough == 0) {
                                door.cur_peak_trough_value = MAX(door.cur_peak_trough_value, door.prev_peak_trough_value);
                                if(door.cur_peak_trough_value < 0.25) {
                                    door.prev_dt = millis();
                                }
                            } else {
...
//							  door.count_peak_trough = 0;
    ...
    *door_status =  NOT_READY;
    ...
    printf("%d %d %f %d %d %f %d %d %d %d %f %f\n", door.prev_dt, door.cur_dt, door.cur_peak_trough_value,
           door.count_peak_trough, door.peak_trough_index, door.cur_pitch, door.cur_ts, NDOF_IsGyroDynamic(),  NDOF_IsAccDynamic(), NDOF_IsAccCalibrated(), raw_gx, *cur_cor_gx);
    return *door_status;
}
