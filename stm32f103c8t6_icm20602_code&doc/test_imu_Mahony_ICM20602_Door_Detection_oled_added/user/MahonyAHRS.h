...
//#define DEFAULT_SAMPLE_FREQ	125f	// sample frequency in Hz
#define twoKpDef	(2.0f * 8.9f)	// 2 * proportional gain
...
void computeAngles()
{
	roll = atan2f(2.*(q1*q2 - q0*q3), 2.*(q2*q3 + q0*q1));
	pitch = asinf(q0*q0-q1*q1+q2*q2-q3*q3);
	yaw = atan2f(2.*(q1*q2 + q0*q3), -2.*(q2*q3 - q0*q1));
	anglesComputed = 1;
}
*/
// XYX
void computeAngles()
{
	roll = atan2f(2.*(q1*q2 + q0*q3), -2.*(q1*q3 - q0*q2));
	pitch = asinf(q0*q0+q1*q1-q2*q2-q3*q3);
	yaw = atan2f(2.*(q1*q2 - q0*q3), 2.*(q1*q3 + q0*q2));
	anglesComputed = 1;
}
//============================================================================================
// END OF CODE
